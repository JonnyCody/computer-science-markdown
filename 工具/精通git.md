...
![image-20220821152235929](./精通git.asset/image-20220821152235929.png)
...
![image-20220821152601879](./精通git.asset/image-20220821152601879.png)
...
![image-20220821152941430](./精通git.asset/image-20220821152941430.png)
...
![image-20220821161931191](./精通git.asset/image-20220821161931191.png)
...
Git和其他版本控制系统的主要差别在于Git对待数据的方式，Git把数据看作是对小型文件系统的一组快照。每次提交更新，或在Git中保存项目状态时，它主要对当时的全部文件制作一个快照并保存这个快照的索引。Git对待数据的方式更像是一个快照流。

![image-20220821164955133](./精通git.asset/image-20220821164955133.png)

当执行Git操作时，几乎只往Git数据库中添加数据，很难让Git执行任何不可逆的操作。
...
![image-20220821170124953](E:\Computer Science\个人笔记\工具\精通git.asset\image-20220821170124953.png)

树结构中有5个对象：3个blob对象（保存文件快照）、一个树对象（记录着目录结构和blob对象索引）以及一个提交对象（包含着指向前述树对象的指针和所有提交信息）

多次提交后，产生的提交对象会包含一个指向上一次提交对象（父对象）的指针。

![image-20220821170517504](E:\Computer Science\个人笔记\工具\精通git.asset\image-20220821170517504.png)

Git的分支，本质上仅仅是指向提交对象的可变指针，Git的默认分支名称是master。
